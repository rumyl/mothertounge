<?php
  require_once "includes/metaheader.php";
  require_once "includes/header.php";
  require_once "includes/sidebar.php";

?>
  <main id="main" class="main">

    <div class="pagetitle">
      <h1>LEKSYON 3</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.php">Home</a></li>
          <li class="breadcrumb-item active">Pahina 31</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    <section class="section dashboard">
      <div class="row">

      <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              <br>
              <h5 class="card-text bold">Ueubrahon 9</h5>
              <p>Pasunura ro mga pamaagi sa pagbuoe it eksamin. Isueat ro numero sa linya.</p>
              <p class="card-text"><input type="text" name="l2_u1_01" style="width:25%;" placeholder="Imong sabat">Basaha ag eobton it mayad ro mga direksyon.<br><input type="text" name="l2_u1_01" style="width:25%;" placeholder="Imong sabat">Isueat ro impurmasyon nga kinahangean pareho it imong pangaean, grado ag seksyon<br><input type="text" name="l2_u1_01" style="width:25%;" placeholder="Imong sabat">Sayura ro nakataeana nga oras para sa eksanim.<br><input type="text" name="l2_u1_01" style="width:25%;" placeholder="Imong sabat">Baliki it uman ro mga pangutana ag sabat.<br><input type="text" name="l2_u1_01" style="width:25%;" placeholder="Imong sabat">Basaha it mayad ro kada aytem bago isueat ro sabat.</p>
              <div style="text-align:right">
                    <input type="submit" name="submit_l2_u1" value="Ipasa ing sabat" style="background:#4154f1;color:white;border-radius:10px;">
            </div>
              <h5 class="card-text bold">Assessment</h5>
              <p><b>A</b>.&nbsp; Isipa nga nagapanaw ka sa <b>camping trip</b> sa sueod it limang adlaw. Maglista it 20 ka tawo, lugar ag mga butang nga gusto mo nga makita o daehon sa lugar.</p>
              <br>
              <img src="assets/img/camp1.png" alt="..." style="width:100%">
              
              <footer class="text-center number">Pahina 31</footer>
            </div>
            
            <div class="navigation-buttons">
                                <a href="page30.php"><button class="btn btn-primary float-left">Nakataliwan</button></a>
                                <a href="student_score.php"><button class="btn btn-primary float-right">Masunod</button></a>
            </div><!-- End navigation-buttons -->
          </div>
        </div>
    </section>

  </main><!-- End #main -->

  
  <?php
  require_once "includes/footer.php";
  ?>
